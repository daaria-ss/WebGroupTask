                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.pos[0] >= 415 and event.pos[0] <= 440:
                        if event.pos[1] >= 10 and event.pos[1] <= 45:
                            text = ''
                            flagmetka = 0
                    elif event.pos[0] >= 160 and event.pos[0] <= 390:
                        if event.pos[1] >= 390 and event.pos[1] <= 440:
                            if flagindex == 0:
                                ...
                                if flagindex == 1:
                                    flagindex = 0
                    elif event.pos[0] >= 470 and event.pos[0] <= 570:
                        if event.pos[1] >= 10 and event.pos[1] <= 45:
                        ...
                        if event.button == 1:
                            try:
                                ...
                                mousex = float(-(325 - mousex))
                                mousey = float((225 - mousey))
                                adress_x = (mousex / 325) * 0.0017 + float(x)
                                adress_y = (mousey / 225) * 0.0006 + float(y)
                                geocoder_request = "http://geocode-maps.yandex.ru/1.x/?apikey=40d1649f-0493-4b70-98ba-98533de7710b&geocode="
                                geocoder_request += str(adress_x)
                                geocoder_request += ','
                                geocoder_request += str(adress_y)
                                geocoder_request += "&format=json"
                                response = requests.get(geocoder_request)
                                if response:
                                    json_response = response.json()
                            ...
                            min_rad = 99999
                            print('Ждите около 15 секунд')
                            for i in range(-50, 51, 20):
                                for j in range(-50, 51, 20):
                                    mousex = float(event.pos[0] + i)
                                    mousey = float(event.pos[1] + j)
                                    mousex = float(-(325 - mousex))
                                    mousey = float((225 - mousey))
                                    adress_x = (mousex / 325) * 0.0017 + float(x)
                                    adress_y = (mousey / 225) * 0.0006 + float(y)
